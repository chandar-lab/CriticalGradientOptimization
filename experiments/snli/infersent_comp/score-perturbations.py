...
def calculate_stats(step):
    ...
    for outp_file in output_files:
        ...
        for pair in d_pairs:
            ...
            rows.append(
                [step, outp_file, pair[0], pair[1], diff_words, bool_ct, i.correct,
                 i.label, j.correct, j.label])
        df = pd.DataFrame(rows,
                          columns=["Step", "Filename", "Pair A Index", "Pair B Index",
                                   "Diff", "Boolean",
                                   "Pair A Correct Label", "Pair A Predicted Label",
                                   "Pair B Correct Label",
                                   "Pair B Predicted Label"])
...
if __name__ == '__main__':
    ...
    parser.add_argument("-e", "--exp_name", type=str, default='exp_seed_11',
                        help='Experiment name')
    parser.add_argument("-s", "--step_name", type=str, default='stepH0',
                        help='step name, comma separared')
