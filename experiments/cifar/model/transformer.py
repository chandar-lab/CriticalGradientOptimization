...
class PositionalEncoder(nn.Module):
    ...
    def __init__(self, emb_dim, max_seq_len=200, dropout=0.1):
        ...
        self.register_buffer('pe',
                             pe)  # block of data(persistent buffer)->temporary memory

    def forward(self, x):
        '''
        inputs:
            x: sequence of vectors of shape
            (batch size, input sequence length, vector dimensions)

        outputs: sequence of vectors of same shape as input with positional
                aka time encoding added to each sample
                shape (batch size, input sequence length, vector dimensions)
        '''
